...
- The app includes `api-trace-core` plus `api-trace-debug` and `api-trace-noop` variants.
- Bootstrap runs in app startup or the main network composition root.
- The real OkHttp path is instrumented.
...
- The output format is JSON from `APITrace.exportJson(pretty)`, not a custom format.
